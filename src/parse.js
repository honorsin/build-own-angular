...
Lexer.prototype.readString = function () {
  ...
  while (this.index < this.text.length) {
    ...
    if (escape) {
      ...
      this.index++;
      this.tokens.push({ text: rawString, value: string });
      return;
    } else if (ch === "\\") {
      escape = true;
    } else {
...
AST.prototype.primary = function () {
  ...
  if (this.expect("(")) {
    ...
    primary = this.constants[this.consume().value];
  } else if (this.expect("{")) {
    primary = this.object();
  } else if (this.peek().identifier) {
    primary = his.identifier();
  } else {
...
ASTCompiler.prototype.compile = function (text) {
  ...
  var fnString =
    this.filterPrefix() +
    "var fn = function(s,l){" +
    (this.state.vars.length ? "var" + this.state.fn.vars.join(",") + ";" : "") +
...
function markConstantAndWatchExpressions(ast) {
  ...
  switch (ast.type) {
    ...
    case AST.MemberExpression:
      ...
      ast.constant =
        ast.object.constant && (!status.computed || ast.property.constant);
      ast.toWatch = [ast];
      break;
...
ASTCompiler.prototype.recurse = function (ast, context, create) {
  ...
  switch (ast.type) {
    ...
    case AST.UnaryExpression:
      return (
        ast.operator + "(" + this.ifDefined(recurse(ast.argument), 0) + ")"
      );
    case AST.BinaryExpression:
...
function ifDefined(value, defaultValue) {
  return typeofvalue === undefined ? defaultValue : value;
}
