...
function $CompileProvider($provide) {
  ...
  this.directive = function (name, directiveFactory) {
    if (_.isString(name)) {
      ...
      _.forEach(
        name,
        function (directiveFactory, name) {
    ...
    function ($injecter, $parse, $rootScope) {
      ...
      function collectDirectives(node, attrs) {
        ...
        if (node.nodeType === Node.ELEMENT_NODE) {
          var normalizedNodeName = directiveNormalize(
          ...
          _.forEach(node.attributes, function (attr) {
            ...
            if (directiveIsMultiElement(directiveNName)) {
              if (/Start$/.test(normalizedAttrName)) {
                ...
                name = name.substring(0, normalizedAttrName.length - 6);
              }
            }
            normalizedAttrName = directiveNormalize(name.toLowerCase());
            addDirective(
      ...
      function compileTemplateUrl(
        ...
        $compileNode,
        attrs,
        {templateDirective: templateDirective,preLinkFns: preLinkFns,postLinkFns: postLinkFns}
      ) {
        ...
        $http.get(templateUrl).success(function (template) {
          ...
          afterTemplateChildLinkFn = compileNodes($compileNodes[0].childNodes);
      ...
      function applyDirectivesToNode(directives, compileNode, attrs, previousCompileContext) {
        ...
        function getControllers(require, $element) {
          if (_.isArray(require)) {
            return_.map(require, getControllers);
          } else {
            var value;
        ...
        function nodeLinkFn(childLinkFn, scope, linkNode) {
          ...
          if (controllerDirectives) {
            _.forEach(controllerDirectives, function (directive) {
              var locals = {
                $scope:
                  directive === newIsolateScopeDirective ? isolateScope : scope,
                $element: $element,
                $attrs: attrs,
              };
