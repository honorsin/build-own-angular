...
describe("$http", function () {
  ...
  it("resolves promise when XHR result received", function () {
    var requestConfig = { method: "GET", url: "http://teropa.info" };
    var response;
  ...
  it("supports functions as header values", function () {
    ...
    var request = { method: "POST", url: "http://teropa.info", data: "42" };
    $http(request);
    $rootScope.$apply();
  ...
  it("makes response headers available", function () {
    var response;
    $http({ method: "POST", url: "http://teropa.info", data: 42 }).then(
      function (r) {
        response = r;
      }
    );
    ...
    expect(response.headersinstanceofFunction).toBe(true);
  ...
  it("allows setting default response transforms", function () {
    ...
    var response;
    $http({ url: "http://teropa.info" }).then(function (r) {
      response = r;
    });
    $rootScope.$apply();
    requests[0].respond(200, { "Content-Type": "text/plain" }, "Hello");
  ...
  it("does not serialize form data for requests", function () {
    ...
    $http({ method: "POST", url: "http://teropa.info", data: formData });
    $rootScope.$apply();
    expect(requests[0].requestBody).toBe(formData);
  });
  it("parses JSON data for JSON responses", function () {
    var response;
    $http({ method: "GET", url: "http://teropa.info" }).then(function (r) {
      response = r;
    });
    $rootScope.$apply();
    requests[0].respond(
  ...
  it("serializes dates to ISO strings", function () {
    ...
    expect(
      /\d{4}-\d{2}-\d{2}T\d{2}%3A\d{2}%3A\d{2}/.test(requests[0].url)
    ).toBeTruthy();
  });
  ...
  it("allows intercepting request errors", function () {
    ...
    var injector = createInjector([
      ...
      function ($httpProvider) {
        ...
        $httpProvider.interceptors.push(
          _.constant({ requestError: requestErrorSpy })
        );
      },
    ]);
    $http = injector.get("$http");
